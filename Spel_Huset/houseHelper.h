...
enum operation { endgame, help, take, open, show, hide, go };
typedef enum operation operationT;
typedef const char *enumOperation[];
#define OPERATIONTEXTS { "avsluta", "hjälp", "tag", "öppna", "visa", "göm", "gå" }
...
typedef struct userInventory inventory;
...
********************************************************************/
int placeObject(MapT , int, int, char, positionT *, int); //int = 0 för random placering.
...
void Kermitmove(MapT *tMap, positionT *pos, int, inventory *userinventory);
